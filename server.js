...
var backend_proxy = function (req, res, opts, cb) {
  if(!backend_host){
    ...
    console.log(req.url);
    console.log(req.url.substring(backend_path.length));
    console.log('http://'+backend_host+req.url.substring(backend_path.length))
    req.pipe(request('http://'+backend_host+req.url.substring(backend_path.length + 1))).pipe(res);
  }
}
